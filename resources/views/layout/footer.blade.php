    <footer class="global-footer">
        <div class="container">
            <div class="row">
                <div class="col-sm-8">
                    <div class="footer-feature-list">
                        <ul class="list-inline">
                            <li><a href="">ユーザー一覧</a></li>
                            <li><a href="">タグ一覧</a></li>
                        </ul>
                    </div>
                    <div class="footer-link-list">
                        <ul class="list-inline">
                            <li><a href="http://blue-corporation.jp/">blue公式サイト</a></li>
                            <li><a href="http://engineer.blue-corporation.jp/">blueエンジニアブログ</a></li>
                        </ul>
                    </div>
                    <div class="footer-copyright">
                        <span>Copyright &copy;{{{ date('Y') }}} blue Inc.</span>
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="footer-form">
                        <form>
                            <div class="footer-form-body">
                                <textarea class="form-control footer-form" name="footer-form" placeholder="Biitaについてご意見、バグを報告してください"></textarea>
                            </div>
                            <div class="footer-form-submit">
                                <button class="btn btn-default btn-block">運営に送信する</button>
                            </div>
                        </form>
                    </div>
                </div>
            </div>
        </div>
    </footer>
